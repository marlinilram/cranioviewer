...
std::vector<double>& NiiLoader::extractSkullVertex(int v, int t, int& num)
{
	...
	for (std::size_t i = 0, e = skullVoxelVal.size(); i != e; ++i) {
		if (skullVoxelVal[i] - v <= t && skullVoxelVal[i] - v >= -t) {
			skullVoxelForIso.push_back(skullVoxel[i*3]);
			skullVoxelForIso.push_back(skullVoxel[i*3+1]);
			skullVoxelForIso.push_back(skullVoxel[i*3+2]);
		}
	}
...
vtkSmartPointer< vtkPolyData > NiiLoader::mcSkullVertex(vector<double> &T_Pts, vector<double> &T_Norms, double isovalue)
{
	int downsample_ritio = 3;
