...
double IcpPointToPoint::fitStep (double *T,const int32_t T_num,Matrix &R,Matrix &t,const std::vector<int32_t> &active) {
  ...
  double D = 0;
  for (int32_t i = 0; i<active.size(); ++i) {
	  Sp += sp.val[0][i]*sp.val[0][i] + sp.val[1][i]*sp.val[1][i] + sp.val[2][i]*sp.val[2][i];
	  D += sp.val[0][i]*q_m.val[i][0] + sp.val[1][i]*q_m.val[i][1] + sp.val[2][i]*q_m.val[i][2];
  }
  float lamb = 10.0;
  //float scale = (D+lamb*active.size())/(Sp+lamb*active.size());
  float scale = D/Sp;
  ...
  // compute point distance between model point and template point
  Matrix p_dist = p_m - p_t;
  double dist = 0;
  for (int32_t i = 0; i<active.size(); ++i) {
	  dist += p_dist.val[i][0]*p_dist.val[i][0] + p_dist.val[i][1]*p_dist.val[i][1] + p_dist.val[i][2]*p_dist.val[i][2];
  }
  dist = dist/(double)active.size();

  // return max delta in parameters
  if (R.l2norm() < 0.4) return -1;
  else {
  if (dim==2) return max((R_-Matrix::eye(2)).l2norm(),t_.l2norm());
  else        return dist;//max((R_-Matrix::eye(3)).l2norm(),t_.l2norm());
  }
}
